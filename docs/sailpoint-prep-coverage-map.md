...
| 2 | Networking & API design | 🟡 Partial | Real REST contracts across Auth/Items/Board (Step 3), a real Gateway doing JWT validation + routing. Also now covered: browser same-origin policy / CORS — a real "Failed to fetch" bug from the web UI (Decision #52), root-caused to Gateway missing `Access-Control-Allow-Origin`, fixed with `CORSMiddleware` and an explicit origin allowlist; preflight `OPTIONS` requests confirmed directly via curl. Not covered: L4 vs L7 load balancing (no real LB at our scale), polling vs SSE vs WebSocket (touched conceptually in the real-time-sync backlog item, not built). |
